...
#
# Paul's changes: 
# removed transforms.CenterCrop(args.resolution) 
# added transforms.Resize(224)
# removed #transforms.CenterCrop(args.resolution),
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    #Data NORMALIZE: subtracts the channel mean and divides by the channel standard deviation
...
def train(train_loader, model, criterion, optimizer, epoch, num_classes, args):
    ...
    f = open("models/output.txt", "a")
    f.write("Epoch: [{:d}] completed, elapsed time {:6.3f} seconds\n".format(epoch, time.time() - epoch_start))
    #f.write("Training Loss: {losses.avg:.4e}\n".format(losses=losses))
    f.write("{losses.avg:.4e}\n".format(losses=losses))
    f.close()
...
def validate(val_loader, model, criterion, num_classes, args):
    ...
    with torch.no_grad():
        ...
        f = open("models/output.txt", "a")
        #f.write("Accuracy: {top1.avg:.3f}\n".format(top1=top1))
        f.write("{top1.avg:.3f}\n".format(top1=top1))
        #f.write("Val Loss: {losses.avg:.4e}\n".format(losses=losses))
        f.write("{losses.avg:.4e}\n".format(losses=losses))
        f.close()
